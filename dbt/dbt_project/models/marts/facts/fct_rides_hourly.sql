-- Aggregate Fact: Rides Hourly
-- Grain: 1 row per ride_hour + city_code + service_type
-- Strategy: insert_overwrite by hourly date partition
...
final as (
    select
        {{ dbt_utils.generate_surrogate_key(['ride_hour', 'city_code', 'service_type']) }} as ride_hourly_sk,
        ride_hour,
        date(ride_hour, '{{ var("reporting_timezone", "Asia/Jakarta") }}') as ride_hour_date,
        cast(format_date('%Y%m%d', date(ride_hour, '{{ var("reporting_timezone", "Asia/Jakarta") }}')) as int64) as ride_date_key,
        city_code,
        service_type,
